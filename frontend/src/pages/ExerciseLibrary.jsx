...
function ExerciseAnimation({ name }) {
  ...
  if (normalized.includes('squat') || normalized.includes('leg press') || normalized.includes('calf raise')) {
    ...
    return (
      <div className="w-full h-48 bg-slate-100/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-550 font-bold uppercase tracking-wider mt-2">
  ...
  if (normalized.includes('curl')) {
    return (
      <div className="w-full h-48 bg-slate-100/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-550 font-bold uppercase tracking-wider mt-2">Bicep Elbow Pivot Isolation</span>
      </div>
    );
  }
  ...
  if (normalized.includes('jogging') || normalized.includes('run')) {
    return (
      <div className="w-full h-48 bg-slate-100/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-550 font-bold uppercase tracking-wider mt-2">Cardio stride Mechanics</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-48 bg-slate-100/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex flex-col items-center justify-center p-4 relative">
      ...
      <span className="text-[10px] text-slate-500 dark:text-slate-550 font-bold uppercase tracking-wider mt-2">Alignment Calibration Guide</span>
    </div>
  );
}
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800/80">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-550" />
            <input
              type="text"
        ...
        <div className="flex gap-1.5 mt-5 overflow-x-auto pb-2 border-t border-slate-250 dark:border-slate-900 pt-4 scrollbar-thin">
          {muscleGroups.map(m => (
            <button
              key={m}
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all border ${
                selectedMuscle === m
                  ...
                  : 'bg-slate-100/60 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800/80 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'
              }`}
            >
              {m}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {loading ? (
            ...
            <div className="col-span-2 py-12 text-center text-slate-500 dark:text-slate-550 text-sm italic">
            ...
            filtered.map(ex => (
              <div 
                ...
                className={`glass-panel p-5 rounded-2xl border cursor-pointer flex justify-between items-center transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-100/40 dark:bg-slate-900/30 ${
              ...
              >
                <div>
                  <h3 className="font-extrabold text-slate-800 dark:text-slate-100 text-sm">{ex.name}</h3>
                  <span className="block text-[11px] text-slate-500 dark:text-slate-400 mt-1">{ex.muscle_group} • {ex.difficulty}</span>
                </div>
                <ChevronRight className={`w-5 h-5 transition-transform duration-200 ${
                  activeExercise?.id === ex.id ? 'translate-x-1 text-cyber-400' : 'text-slate-500 dark:text-slate-500'
                }`} />
              </div>
            ))
          )}
        ...
        <div className="md:sticky md:top-6">
          {activeExercise ? (
            <div className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800/80 space-y-5 shadow-glass animate-fade-in">
              
              {/* Header */}
              <div className="flex justify-between items-start border-b border-slate-250 dark:border-slate-900 pb-3.5">
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">{activeExercise.name}</h3>
                  <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{activeExercise.muscle_group} • {activeExercise.difficulty}</span>
                </div>
                <button 
                  onClick={() => setActiveExercise(null)}
                  className="p-1 hover:bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-550 hover:text-slate-700 dark:text-slate-200 rounded-lg transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Posture Demo</span>
                <ExerciseAnimation name={activeExercise.name} />
              </div>

              {/* Instructions */}
              <div className="space-y-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Instructions</span>
                <ol className="list-decimal pl-4.5 text-xs text-slate-700 dark:text-slate-300 space-y-1.5 leading-relaxed">
              ...
              {/* Warnings/Tips Panel */}
              <div className="space-y-3 pt-3 border-t border-slate-250 dark:border-slate-900">
                {/* Mistakes */}
                <div className="flex gap-2.5 p-3 rounded-xl bg-red-950/20 border border-red-500/25">
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-3 text-[11px] text-slate-700 dark:text-slate-300 mt-1 space-y-0.5">
            ...
            <div className="glass-panel p-8 rounded-3xl border border-slate-200 dark:border-slate-800/60 text-center text-slate-500 dark:text-slate-550 py-16 flex flex-col items-center">
              <BookOpen className="w-8 h-8 text-slate-500 dark:text-slate-500 mb-3" />
              <span className="text-sm font-semibold">Select an Exercise</span>
              <span className="text-xs text-slate-500 dark:text-slate-500 mt-1 max-w-[200px] leading-relaxed">
